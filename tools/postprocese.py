...
from lib.core.postprocess import morphological_process, connect_components_analysis, fillHole
...
def postprocess(cfg,opt):
    ...
    for path, img, im0s, vid_cap,shapes in dataset:
        ...
        da_seg_mask = da_predict.float()
        da_seg_mask = tf(da_seg_mask.cpu())
        ...
        ll_seg_mask = ll_predict.float()
        ll_seg_mask = tf(ll_seg_mask.cpu())
        ll_seg_mask = ll_seg_mask.int().squeeze().cpu().numpy()

        t1 = time_synchronized()
        ll_seg_set = np.vstack(np.where(ll_seg_mask==1)).T
        print(ll_seg_set.shape)
        y_pred = DBSCAN(eps = 3, min_samples = 3).fit_predict(ll_seg_set)
        print(type(y_pred))
        print(np.unique(y_pred))
        plt.scatter(ll_seg_set[:, 1], ll_seg_set[:, 0], c=y_pred)
        for class_idx in np.unique(y_pred):
            x=np.array([ll_seg_set[:, 1][i] for i in np.where(y_pred == class_idx)])
            y=np.array([ll_seg_set[:, 0][i] for i in np.where(y_pred == class_idx)])
            print(x.shape)
            x= x.flatten()
            y =y.flatten()
            print(x.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/zwt/wd/DaChuang/runs/BddDataset/_2021-03-12-00-19/epoch-67.pth', help='model.pth path(s)')
    parser.add_argument('--source', type=str, default='inference/image_test', help='source')  # file/folder, 0 for webcam

...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    #print(prediction[..., 4].shape) #[1, 57960]
    xc = prediction[..., 4] > conf_thres  # candidates [bs, 57960]
    # Settings
    ...
    t = time.time()
    output = [None] * prediction.shape[0] #bs
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence [57960, 85]
        ...
        boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), scores  不同类错开
